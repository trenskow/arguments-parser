		...
		const result = {
			...
			then: (
				...
				return (async () => {
					...
					const complete = async (current = args[autocompleteIndex] || '') => {
						...
						if (schemas[autocompleteIndex]) {
							...
							if (schemas[autocompleteIndex].enum) {
								return Object.keys(schemas[autocompleteIndex].enum)
									.map((item) => ({
										type: 'option',
							...
							return await schemas[autocompleteIndex].autocomplete?.(current) || [];
					...
					for (idx = 0; idx < autocompleteIndex || idx < args.length; idx++) {
						...
						if (!schemas[idx]) {
							...
							if (nonOptions || !(args[idx] || '').startsWith('-')) {
								continue;
							}
