...
import { default as isvalid, merge, keyPaths } from 'isvalid';
		...
		command: async (commands) => {
			...
			helpOptions = () => {
				...
				const tools = Object.entries(commands).map(([key, command]) => ({
					name: `${caseit(key, 'kebab')}${Array.isArray(command.parameters) ? ` ${command.parameters.map((parameter) => `${opening}${caseit(parameter, 'kebab')}${closing}`).join(' ')}` : ''}`,
						...
						list(print, Object.fromEntries(allKeyPaths
							.map((keyPath) => {
								...
								if (typeof keyPathSchema.enum !== 'undefined') {
									description.push(`(${opening}${keyPath}${closing}: ${Object.keys(keyPathSchema.enum).map((value) => `\`${caseit(value, 'kebab')}\``).join(', ')})`);
								}
...
export { buildIns as autocompleteBuildIns } from './autocomplete/index.js';
